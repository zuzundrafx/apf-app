// src/components/StyleModal.tsx – финальная версия с прозрачностью вместо удаления из DOM
  ...
  return (
    <div className="rewards-modal-overlay" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      >
        ...
        >
          {/* Striker — всегда в DOM, прозрачность вместо удаления */}
          <div 
            style={{ 
              display: 'flex', 
              flexDirection: 'column', 
              ...
              opacity: (!isConfirming || selectedStyle === 'striker' || currentStyle === 'striker') ? 1 : 0,
              pointerEvents: (!isConfirming || selectedStyle === 'striker' || currentStyle === 'striker') ? 'auto' : 'none',
          ...
          {/* Grappler — всегда в DOM, прозрачность вместо удаления */}
          <div 
            style={{ 
              display: 'flex', 
              flexDirection: 'column', 
              alignItems: 'center', 
              gap: 'clamp(8px, 2vh, 16px)',
              opacity: (!isConfirming || selectedStyle === 'grappler' || currentStyle === 'grappler') ? 1 : 0,
              pointerEvents: (!isConfirming || selectedStyle === 'grappler' || currentStyle === 'grappler') ? 'auto' : 'none',
              transition: 'opacity 0.3s ease'
            }}
          >
            <div 
